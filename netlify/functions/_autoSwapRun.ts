// ---------------------------------------------------------------------------
// The auto-swap run itself, shared by both entry points.
//
// It lives here rather than in auto-swap.mts because a Netlify SCHEDULED
// function is not served over HTTP — declaring `config.schedule` makes its URL
// return an empty 404. The manual dry-run and test-email paths were on that
// function, so they were unreachable in production and failed with
// "Unexpected end of JSON input" from an empty body.
//
// So: auto-swap.mts carries the schedule, auto-swap-test.mts is an ordinary
// HTTP function, and both call runAutoSwap() — one implementation, no drift.
//
...
export async function runAutoSwap(req?: Request): Promise<Response> {
  ...
  try {
    ...
    const maintenance = computeMaintenance({
      ...
      recovering,
      tagMap,
      campaignTagsById: assignments.byCampaign,
      settings,
    });
